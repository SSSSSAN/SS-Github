...
def learning():
    ...
    in_output_sc_model  = StandardScaler() #データの標準化
    ...
    def save_regression(model_raw, model_std, model_name):
        ...
        train_output_predict_stdtoraw    = model_std.predict(list_train_std[in_n], verbose = 0)
        test_output_predict_stdtoraw     = model_std.predict(list_test_std[in_n], verbose = 0)
    ...
    def bayesian_optimization(model_raw, model_name): #全探索ができないくらい説明変数が多いときにある望みの目的変数を得るために逆探索
        if is_bayesian_opt == True:
            ...
            def function_for_bayesian(x):
                ...
                if optimize_type == 0:
                    #max
                    return model_raw.predict(x)[0][-1] * -1 #[0][-1]：0番目の後ろから1つ目の値を取得
    ...
    if is_dl == False:
        ...
        def objective_dl(trial): #後にoptunaを使用してパラメーターを最適化し、再度予測
